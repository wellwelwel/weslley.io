...
const CATALOG = 'src/helpers/variants.json';
...
const shapeOf = async (path: string): Promise<Shape> => {
  const meta = await sharp(path).metadata();

  if (!meta.width || !meta.height)
    throw new Error(`Unreadable dimensions: ${path}`);
...
const walk = async (directory: string): Promise<string[]> => {
  ...
  const nested = await Promise.all(
    entries.map((entry) => {
      ...
      return Promise.resolve(RASTERS.has(extname(entry.name)) ? [path] : []);
    })
  );
