...
tags: [Testes, Testes Unitários, Node.js]
social: ./banner.png
---

If you're a beginner developer (_or not_), I want to show you that testing can be simple and that complexity only comes according to our own needs.
...
<ul>
  <li>
    <Modal trigger="Jest" title={<><img src="/img/jest.svg" alt="Jest icon" /> Jest</>}>
      **Jest** é um framework de testes JavaScript mantido pelo **Meta** (Facebook), com foco em oferecer uma solução **completa e integrada**.

      Possui recursos integrados como **mocking automático**, **cobertura de código** e **snapshot testing**, sendo especialmente popular em projetos **React**.

      Por ser uma solução all-in-one, pode exigir **configurações complexas** para projetos modernos (especialmente **ESM** e **TypeScript**), além de ter **performance inferior** comparado a alternativas mais modernas.
    </Modal>

  </li>
  <li>
    <Modal trigger="Mocha" title="Mocha">
      **Mocha** é um test runner **minimalista e flexível** para Node.js, focado em **simplicidade** e **extensibilidade**.

      Diferente do Jest, o Mocha não inclui biblioteca de asserções ou mocking, permitindo que você escolha suas próprias ferramentas (geralmente usado com **Chai** para asserções e **Sinon** para mocks).

      É ideal para quem prefere **controle total** sobre o ambiente de testes, mas exige mais configuração manual.
    </Modal>

  </li>
  <li>
    <Modal trigger="Vitest" title="Vitest">
      **Vitest** é um test runner **moderno**, desenvolvido pela mesma equipe do **Vite**.

      Possui API **compatível com Jest**, facilitando migração, e oferece suporte nativo para **ESM**, **TypeScript** e **HMR** (Hot Module Replacement).

      Por estar acoplado ao **Vite**, pode não ser ideal para todos os projetos, especialmente aqueles que não usam bundlers ou preferem outras ferramentas de build, tendo **foco maior em frontend** (embora funcione em backend), e **não oferece suporte para CommonJS (CJS)**.
    </Modal>
